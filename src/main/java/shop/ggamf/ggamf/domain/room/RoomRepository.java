...
public interface RoomRepository extends JpaRepository<Room, Long> {
    ...
    // 추천 겜프 목록 - 내가 닫은 방 목록
    @Query("select r from Room r join fetch r.user u where r.user.id = :userId and r.active=false order by r.updatedAt desc")
    List<Room> findByUserLatest(@Param("userId") Long userId);
