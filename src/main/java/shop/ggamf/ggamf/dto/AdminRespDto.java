...
public class AdminRespDto {
    ...
    //select r.id, u.name badUsername, r.created_At, rs.reason, r.detail, u.name submitUsername, (select count r.bad_user_id) form report r where bad_user_id = :badUserId) from report r inner join users u on r.bad_user_id = u.id inner join reason_code rs on rs.id = r.reason_code_id where r.bad_user_id = :badUserId;
}
